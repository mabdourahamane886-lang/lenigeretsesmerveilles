...
export default async function AdminMedias() {
  ...
  const [{ data: regions }, { data: wonders }, { data: media }] = supabase
    ? await Promise.all([
        ...
        supabase
          .from('niger_media')
          .select('id,title,url,credit,published,region_id,media_type,media_category')
    ...
    <main className="section">
      <div className="container" style={{ paddingBottom: 100 }}>
        ...
        <div className="adminHero">
          <div>
            <div className="kicker">Médias</div>
            <h1>Galerie du Niger</h1>
            <p className="muted">
              Ajoute une photo ou une vidéo comme dans WhatsApp : caméra, vidéo,
              galerie ou bouton +. Le fichier est envoyé vers Smooth Bundle puis
        ...
        <form className="adminForm" action={createMedia} encType="multipart/form-data">
          <label>
            Titre
            <input required name="title" placeholder="Grande Mosquée d’Agadez" />
          </label>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            >
              <summary style={{ cursor: 'pointer', fontWeight: 850, padding: '10px 2px' }}>
                ➕ Plus d’options
              </summary>
              <div
                style={{
              ...
              >
                <span>
                  📎 Galerie : « Photo — Galerie » et « Vidéo — Galerie » ouvrent la
                  galerie/fichiers de ton appareil.
                </span>
                <span>
                  📱 Caméra : les boutons dédiés ouvrent directement la caméra sur les
          ...
          <label>
            Ou URL du média
            <span className="optional">Optionnel si tu utilises Caméra, Vidéo ou Galerie</span>
          ...
          <label className="check">
            <input type="checkbox" name="published" defaultChecked /> Publier immédiatement
          </label>
          ...
          {(media || []).map((item) => (
            <article className="card" key={item.id}>
              ...
              <div className="cardbody">
                <span className="tag">{item.published ? 'Publié' : 'Brouillon'}</span>
                <h3>{item.title}</h3>
